...
void ofApp::setup(){
	...
	motionColor.setup(ofxMotion::DrawMode::RECT, vec2(400, 50), vec2(1.0, 1.0), 275, 275, 0.0f, ofColor(ofRandom(0, 255), ofRandom(0, 255), ofRandom(0, 255)), ofxMotion::AnchorMode::ANCHOR_TOP_LEFT, true);
	...
	for (int i = 0; i < 5; i++) {
		...
		motionRadialNoise[i].getMotionTransform()->startRadialNoise(275, 0.425, vec2(960, 200), 60.0);
	}
...
void ofApp::timer(float now) {
	...
	if (timer >= timerSleepTime) {
		...
		motionMoveLiner[0].getMotionTransform()->startMoveLiner(vec2(50, 50), vec2(250, 50), 0.5, 0, ofxeasing::quart::easeOut);
		motionMoveLiner[1].getMotionTransform()->startMoveLiner(vec2(50, 100), vec2(250, 100), 0.5, 0, ofxeasing::quad::easeOut);
		motionMoveLiner[2].getMotionTransform()->startMoveLiner(vec2(50, 150), vec2(250, 150), 0.5, 0, ofxeasing::linear::easeNone);
		motionMoveLiner[3].getMotionTransform()->startMoveLiner(vec2(50, 200), vec2(250, 200), 0.5, 0, ofxeasing::quart::easeIn);
		motionMoveLiner[4].getMotionTransform()->startMoveLiner(vec2(50, 250), vec2(250, 250), 0.5, 0, ofxeasing::exp::easeIn);
		motionMoveLiner[5].getMotionTransform()->startMoveLiner(vec2(50, 300), vec2(250, 300), 0.5, 0, ofxeasing::back::easeIn);
		...
		motionMoveCurve[0].getMotionTransform()->startMoveCurve(vec2(62, 450), vec2(675, 450), 100, -1, 1.5, 0, ofxeasing::quart::easeOut);
		motionMoveCurve[1].getMotionTransform()->startMoveCurve(vec2(62, 500), vec2(675, 500), 100, -1, 1.5, 0, ofxeasing::quad::easeOut);
		motionMoveCurve[2].getMotionTransform()->startMoveCurve(vec2(62, 550), vec2(675, 550), 100, -1, 1.5, 0, ofxeasing::linear::easeNone);
		motionMoveCurve[3].getMotionTransform()->startMoveCurve(vec2(62, 600), vec2(675, 600), 100, 1, 1.5, 0, ofxeasing::quart::easeIn);
		motionMoveCurve[4].getMotionTransform()->startMoveCurve(vec2(62, 650), vec2(675, 650), 100, 1, 1.5, 0, ofxeasing::exp::easeIn);
		motionMoveCurve[5].getMotionTransform()->startMoveCurve(vec2(62, 700), vec2(675, 700), 100, 1, 1.5, 0, ofxeasing::back::easeIn);
